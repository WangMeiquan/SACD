...
def sacd_config(model,tokenizer,prompts,head_config,token_ranges=[],scale=0.01,scale_position="include",max_length=2048,task='hellaswag',flag='None'):
    mt = ModelAndTokenizer(model=model, tokenizer=tokenizer)

    sacd_head_config=head_config
    if sacd_head_config[0]==None:
        head_config = None
    else:
        head_config = read_head_config(sacd_head_config[0])
    
    if sacd_head_config[1]==None:
        head_config_inner =None
    else:
        head_config_inner =read_head_config(sacd_head_config[1])
